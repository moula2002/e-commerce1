// src/components/category/Fashion.jsx
...
function Fashion() {
  ...
  useEffect(() => {
    const fetchFashionData = async () => {
      try {
        // 🔹 Step 1: Find category named "Fashion"
        const categoryRef = collection(db, "category");
        const q = query(categoryRef, where("name", "==", "Fashion"));
        const categorySnapshot = await getDocs(q);

        if (categorySnapshot.empty) {
          throw new Error("Fashion category not found in Firestore!");
        }
  ...
  return (
    <Container className="my-5 text-center">
      ...
      <p className="text-muted mb-5">
        {category?.description || "Explore our latest fashion collection!"}
      </p>
