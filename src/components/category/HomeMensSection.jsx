...
// 🎨 DEFINE CONSTANTS FOR MODERN AESTHETICS (Consistent)
...
// 🎨 Custom CSS for this component (HEIGHT REDUCED & MOBILE OPTIMIZED - Matches Accessories)
const customStyles = {
...
  imageContainer: (isMobile) => ({ // 📱 MOBILE ADJUSTMENT: Added dynamic height
    width: "100%",
...
    maxHeight: "100%",
    objectFit: "contain",
    transition: "transform 0.3s ease-in-out", // ⬇️ FASTER TRANSITION (was 0.4s)
...
    fontWeight: "900",
    color: PRIMARY_TEXT_COLOR,
    letterSpacing: "-1.5px", // ⬇️ HEIGHT REDUCTION: Reduced letter spacing (was -1.8px)
...
  },
  viewDealButtonHover: {
    backgroundColor: SALE_COLOR, // 🎨 CHANGED TO SALE_COLOR HOVER (Matches Accessories)
    borderColor: SALE_COLOR,    // 🎨 CHANGED TO SALE_COLOR HOVER (Matches Accessories)
    transform: "translateY(-2px)",
    boxShadow: `0 5px 15px ${SALE_COLOR}80`, // 🎨 CHANGED TO SALE_COLOR SHADOW (Matches Accessories)
  },
  exploreButton: {
...
// 💅 Hover Effects Logic (ADJUSTED FOR LESS LIFT/SHADOW - Matches Accessories)
const handleCardMouseEnter = (e) => {
  e.currentTarget.style.transform = "translateY(-8px)"; // ⬇️ REDUCED LIFT (was -12px)
  e.currentTarget.style.boxShadow = "0 20px 40px rgba(0, 0, 0, 0.15)"; // ⬇️ REDUCED SHADOW (was 0.25)
...
// 🌟 Dummy Product Generator (Adjusted base price to match Accessories)
...
  const [isMobile, setIsMobile] = useState(window.innerWidth <= 576); // 📱 MOBILE ADJUSTMENT: Added state

  useEffect(() => { // 📱 MOBILE ADJUSTMENT: Added resize handler
...
          originalPrice: doc.data().originalPrice ? Number(doc.data().originalPrice) : 1499,
        }));
...
        
        {/* 🌟 ATTRACTIVE HEADER (MODIFIED TO BE COMPACT - Matches Accessories) */}
...
            {/* Custom Underline Element (from Accessories) */}
...
            Discover the latest trends in men's clothing — elevate your everyday style!
          </p>
        </div>
