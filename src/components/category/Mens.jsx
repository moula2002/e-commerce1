// src/components/category/Mens.jsx
...
function Mens() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1️⃣ Fetch category by name "Mens"
        const categoryRef = collection(db, 'category');
        const categoryQuery = query(categoryRef, where('name', '==', 'Mens'));
  ...
  return (
    <Container className="my-5 text-center">
      <h2 className="fw-bold text-dark mb-4">{category?.name || "Men's"} Collection 👔</h2>
      ...
      <p className="text-muted mb-5">
        {category?.description || "Discover the latest trends and essential styles for men."}
      </p>
      ...
      {products.length > 0 ? (
        ...
        <div className="p-4 bg-primary bg-opacity-10 rounded">
          <p className="text-primary fw-bold mb-0">No products found for the {category?.name || 'Men\'s'} category yet.</p>
        </div>
      )}
    </Container>
  );
