...
function Stationary() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1️⃣ Fetch category by name "Stationary"
        const categoryRef = collection(db, 'category');
        const categoryQuery = query(categoryRef, where('name', '==', 'Stationary'));
        const categorySnapshot = await getDocs(categoryQuery);

        if (categorySnapshot.empty) {
          throw new Error('Stationary category not found!');
        }
        ...
        console.error('Error fetching Stationary data:', err);
  ...
  if (loading) return (
    <Container className="text-center my-5">
      ...
      <p>Loading Stationary Category...</p>
    </Container>
  );
  ...
  return (
    <Container className="my-5 text-center">
      <h2 className="fw-bold text-dark mb-4">{category?.name || "Stationary"} Collection</h2>
      ...
      <p className="text-muted mb-5">
        {category?.description || "Everything you need for school and office supplies!"}
      </p>
      ...
      {products.length > 0 ? (
        ...
        <div className="p-4 bg-primary bg-opacity-10 rounded">
          <p className="text-primary fw-bold mb-0">No products found in this category yet.</p>
        </div>
      )}
    </Container>
  );
